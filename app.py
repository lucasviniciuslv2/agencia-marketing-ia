...
# ==========================================
# MOTOR VISUAL DO ESCRITÓRIO (CORRIGIDO)
# ==========================================
def render_office(status_agentes, selecionados):
    ...
    for ag in agentes_config:
        ...
        glow_class = ""
        opacidade = "1" if foi_selecionado else "0.2"
        cor_monitor = "#1e1e1e"
        status_texto = "💤"
    ...
    return f"""
    <style>
        /* O ERRO ESTAVA AQUI: grid-template-columns com hífen correto */
        .office-grid {{
            display: grid; 
        ...
        .name-tag {{
            ...
            font-size: 11px; font-family: sans-serif; margin-bottom: 5px; border: 1px solid #333;
            white-space: nowrap;
        }}
        .mesa {{
...
with escritorio_container:
    # Aumentei um pouco o height para garantir que nada corte
    ...
    if not groq_key or not tema:
        ...
        for id_ag, nome_ag, task_desc in agentes_jobs:
            if dict_selecionados[id_ag]:
                ...
                ts = Task(description=task_desc, expected_output="Um parágrafo de resultado.", agent=ag)
        ...
        st.success("Equipe finalizou o trabalho!")
